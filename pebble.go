...
func NewPebbleDB(name string, dir string) (*PebbleDB, error) {
	...
	opts.MemTableSize = 256 << 20        // 512 MB per memtable
	opts.MemTableStopWritesThreshold = 4 // 2 GB of memtables

	// Cache size (used for reads/writes; balance with available RAM)
	opts.Cache = pebble.NewCache(256 << 20) // 256 MB cache

	opts.Levels = []pebble.LevelOptions{
		{
			TargetFileSize: 64 << 20, // Larger SSTables (128 MB per file)
		},
	}
